...
const Section = ({ children, className, id }: SectionProps) => {
  return (
    <section className={cn("py-6 md:py-8", className)} id={id}>
      {children}
    </section>
  );
};
...
const Container = ({ children, className, id }: ContainerProps) => {
  return (
    <div className={cn("mx-auto max-w-5xl", "p-4 sm:p-6", className)} id={id}>
      {children}
    </div>
  );
};
...
const Article = ({ children, className, id }: ArticleProps) => {
  return (
    <article
      className={cn(
        // General Prose
        "prose:font-sans prose prose-neutral dark:prose-invert xl:prose-lg max-w-none",
        // Prose Headings
        "prose-headings:font-normal",
        // Prose Paragraphs
        "prose-p:mb-0",
